...
char *tftp_errorcode_string[] = { "not defined", "file not found",
		"access violation", "disk full", "illegal operation",
...
int tftp_send_data_packet(int sd, struct sockaddr_in *to, int block, char *buf,
		int buflen) {
...
int tftp_process_read(int sd, struct sockaddr_in *to, char *fname) {
	...
	while (1) {
		...
		for (retries = 0, ackreceived = 0;
				!ackreceived && retries < TFTP_MAX_RETRIES; retries++) {
			/* send the data */
...
int tftp_process_write(int sd, struct sockaddr_in *to, char *fname) {
	...
	while (1) {
		...
		for (retries = 0, data_received = 0;
				!data_received && retries < TFTP_MAX_RETRIES; retries++) {
			/* send the ack */
			...
			if (lwip_select(sd + 1, &rfds, NULL, NULL, &tv) > 0) {
				...
				datalen = lwip_recvfrom(sd, buf, TFTP_DATA_PACKET_LEN, 0,
						(struct sockaddr *) &from, (socklen_t *) &fromlen);
		...
		if (!data_received) { /* we did not receive data for all the retries */
			...
			n = mfs_file_write(fd, buf + TFTP_DATA_PACKET_HDR_LEN,
					datalen - TFTP_DATA_PACKET_HDR_LEN);
			if (n != 1) {
...
void tftpserver_application_thread(void *p) {
	int sd;
	int port = tftp_port;
	struct sockaddr_in server;
	int fromlen, n;
...
void print_tftp_app_header() {
	xil_printf("%20s %6d %s\r\n", "tftp server", tftp_port,
			"use command $ tftp -i <board_IP> \r\n tftp>PUT <source-file> to transfer a file to server"
			"\r\n tftp> GET <file> to get a file from server");
}
